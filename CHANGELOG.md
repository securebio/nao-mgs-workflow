...
- Mark a read with one mate unaligned as a duplicate of a complete pair sharing its 5′ coordinate and strand, as `samtools markdup` does. The read joins the nearest such pair only, so a shared coordinate cannot merge groups of pairs, and it never becomes the group exemplar. Reads attached this way count towards `prim_align_dup_count` but are excluded from `prim_align_dup_pairwise_match_frac`. Pairs split across two genomes are keyed under both at once and are never attachment targets. (#1012)
