...
- Validate the host-infection overrides JSON against a documented schema; malformed input raises a clear `ValueError` instead of a `KeyError`/`TypeError` deep in the loader. Also warns on no-op entries (`hosts: []`) and on duplicate `(taxid, host)` pairs across multiple entries (keeping the first occurrence).
