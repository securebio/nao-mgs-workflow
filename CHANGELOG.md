...
- Swap `zcat` for `rapidgzip -P ${task.cpus}` in `COUNT_READS` for true parallel gzip inflate (~6-8× faster than zcat on standard BCL convert output, ~3× faster than pigz); bump from `single` to `small` (8 cpus). Adds `rapidgzip=0.15.2` to the `coreutils_gzip_gawk` container. Out-of-scope swap of pigz→rapidgzip on the other decompression hot paths is tracked in #776.
