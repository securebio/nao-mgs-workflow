...
- Swap `zcat` for `rapidgzip --count-lines -P ${task.cpus}` in `COUNT_READS` for true parallel gzip inflate (~6-8× faster than zcat on standard BCL convert output, ~3× faster than pigz). Introduces a new `count_reads_resources` tier with an input-size-aware cpus closure (1 cpu for total gzipped R1+R2 ≤ 2 GB; 8 cpus above) so small samples get rapidgzip's single-thread inflate win (~3.8× over zcat) without over-allocating cores, while production-scale samples get the full parallel-inflate speedup. Adds `rapidgzip=0.15.2` to the `coreutils_gzip_gawk` container. Out-of-scope swap of pigz→rapidgzip on the other decompression hot paths is tracked in #776.
