...
- Ignore the newly-published gzip `CVE-2026-41992` (HIGH, no Debian trixie fix; gzip is a base-layer package and the pipeline never decompresses untrusted `.lzh` archives) to keep the `scan-containers` check green, and document the `nucleaze_k` and `viral_taxids_exclude_hard` INDEX parameters in `docs/config.md`.
