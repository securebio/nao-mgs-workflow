...
- `benchmark-downstream`: emit a `findings.tsv` coverage manifest (every required finding — threshold flags plus non-threshold triggers like clades reaching zero, severe reassignments, and output/schema anomalies — as one named, routed row) and a `bounding_numbers.tsv` for the Checked section, and precompute per-finding drivers (dominant gained/lost taxon, Kraken `mover_rank`, reassignment `is_severe`/`is_dominant`, clade `reaches_zero`, BLAST `is_agreement_driver`). Moves finding-coverage and driver lookup from the report author into deterministic columns so concentrated drivers and sub-threshold findings are not missed. Developer/agent tooling only.
