...
- The INDEX workflow now publishes `virus-genome-metadata-raw.tsv.gz`, the full set of viral assemblies enumerated by `datasets summary` (columns: `assembly_accession`, `taxid`, `organism_name`, `source_database`, `assembly_status`), captured before the host-infection and assembly-status filters drop genomes. This records each assembly's build-time taxid so downstream tooling (e.g. index benchmarking) can attribute why a genome was or wasn't included without live NCBI lookups. New published output only; no existing output changed and no RUN/DOWNSTREAM consumer reads it.
