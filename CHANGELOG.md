...
- Rework `MAKE_VIRUS_GENOME_DB` to enumerate viral accessions up-front via a single `datasets summary` call (new `ENUMERATE_VIRAL_ACCESSIONS` module), filter by host-infection and assembly status, chunk filtered accessions into N-line files, and fan out `DOWNLOAD_VIRAL_GENOMES` per chunk on `/scratch` using accession-mode `datasets download`. Drops the `taxonomy_nodes` input and per-taxid taxon-mode downloads in favor of accession-mode downloads keyed off the up-front enumeration; adds `viral_accession_chunk_size` config parameter (default 10000). Final published `fasta`/`metadata` outputs preserve the same column schemas, but the set of genomes selected may differ in edge cases (e.g. when an accession's taxid changes between the summary and download calls), so results are not guaranteed directly comparable across this boundary.
