...
- `MASK_GENOME_FASTA` now adds a third masking pass that masks host ribosomal RNA (rRNA) contamination out of the viral genome reference, fixing host rRNA reads being mis-flagged as viral when a viral genome record carries an embedded rRNA stretch. The pass runs BBDuk against the existing `ribo-ref-concat.fasta.gz` (threaded through `MAKE_VIRUS_GENOME_DB`; `INDEX` reordered so `JOIN_RIBO_REF` runs before `MAKE_VIRUS_GENOME_DB`, no new download) with `k=24` (matching the `EXTRACT_VIRAL_READS` k-mer screen), `hdist=0`, `mask=N`, `rcomp=t`, relying on BBDuk treating `U` as `T` so the RNA reference masks DNA genome regions directly. This masks ~0.035% of the reference (~0.9 Mb), changing the contents of `virus-genomes-masked.fasta.gz` and therefore `EXTRACT_VIRAL_READS` results on the next index rebuild; no outputs renamed or removed. Adds a `virus-genomes-mask-rrna.stats.txt` BBDuk stats log (#847).
