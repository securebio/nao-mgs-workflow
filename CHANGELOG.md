...
- Extend the default `viral_taxids_exclude_hard` list to suppress three additional groups that are routinely flagged human/vertebrate-infecting by upstream Virus-Host-DB but are not human/vertebrate pathogens: Microviricetes (class, taxid `10841`; covers Microviridae bacteriophages frequently deposited with `Homo sapiens` as the source-sample host), Smacoviridae (family, taxid `2169574`), and Picobirnaviridae (family, taxid `585893`). The previous list excluded only the three large dsDNA bacteriophage classes (Caudoviricetes, Malgrandaviricetes, Faserviricetes); benchmarking the 20260518 index against 20250825 surfaced ~37 species-level Microviridae newly status=1 for `human` (e.g. multiple `Microvirus D_HF*` and `Human gut microviridae` entries) plus existing smacovirus/picobirnavirus false positives. Applied to all three INDEX config files (`configs/index.config`, `configs/index-for-run-test.config`, `tests/configs/index.config`); also fixes a longstanding typo in the inline comment (`Malgrantaviricetes` → `Malgrandaviricetes`).
