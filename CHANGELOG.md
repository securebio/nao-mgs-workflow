...
- Replace clustering with downsampling in DOWNSTREAM's post-hoc viral validation: each per-species partition is now downsampled to at most `params.validation_n_sample` reads by a deterministic bottom-N hash sample on `seq_id`, those reads are BLASTed, and every hit carries its own validation result rather than one inherited from a cluster representative. This changes the schema and contents of `validation_hits.tsv.gz`.
    ...
    - Replace the `validation_cluster_identity` and `validation_n_clusters` parameters with `validation_n_sample` (default `1000` for short reads, `1000000` for ONT), and drop the injected `cluster_min_len`.
