...
- Pad accession chunk filenames in `FILTER_VIRAL_GENBANK_METADATA` to the chunk count rather than a fixed four digits. Past 9,999 chunks `chunk_10000.txt` sorted before `chunk_9999.txt`, and downstream steps order chunks by filename, so the genome FASTA would have been concatenated out of accession order and `seqkit rmdup` would have kept a different copy of a repeated sequence. Names are unchanged at 9,999 chunks or fewer.
- Stream the metadata in `PREPARE_VIRAL_METADATA` instead of reading every row into memory, cutting peak memory roughly threefold (1.0 GB to 0.3 GB at a million rows). Output is unchanged, and a failed run no longer leaves a partly expanded table behind.
