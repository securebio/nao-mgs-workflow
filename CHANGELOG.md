...
- Size GNU `sort`'s buffer explicitly in `SORT_FILE` and `SORT_FASTQ`, fixing slow sorts and `No space left on device` failures on large inputs. Both modules stream a gzipped file through `sort`, which sizes its buffer from the input's `st_size`; a pipe has none, so `sort` fell back to its small built-in floor regardless of the task's memory reservation, split large inputs into many sorted runs needing several 16-way merge passes, and held about one uncompressed copy of the input in spill files on the instance's local disk for the duration. A new `lib/SortUtils.groovy` derives `--buffer-size` from `task.memory`, pins `--parallel` and `--batch-size`, points `--temporary-directory` at a task-local spill directory, and compresses spill files; new `sort_file_resources`/`sort_fastq_resources` resource labels tier cpus and memory on the input size in place of the static `single` label. Sorted output is unchanged: `sort`'s last-resort whole-line comparison makes the ordering total, so buffer size, parallelism, merge fan-in and spill compression cannot affect the result. (#924)
