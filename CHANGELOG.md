...
- `MASK_GENOME_FASTA` now adds a third masking pass that masks host ribosomal RNA (rRNA) contamination out of the viral genome reference, fixing host rRNA reads being mis-flagged as viral when a viral genome record carries an embedded rRNA stretch. The pass runs BBDuk against the existing `ribo-ref-concat.fasta.gz` (threaded through `MAKE_VIRUS_GENOME_DB`; `INDEX` reordered so `JOIN_RIBO_REF` runs before `MAKE_VIRUS_GENOME_DB`, no new download) at the `EXTRACT_VIRAL_READS` k-mer screen size (`nucleaze_k`, threaded through so the mask k can't drift from the screen k), with `hdist=0`, `mask=N`, `rcomp=t`, relying on BBDuk treating `U` as `T` so the RNA reference masks DNA genome regions directly. Measured against the current production index, this masks ~0.015% of the reference (~0.36 Mb across ~13% of genome records) — the masked `N`s feed both the `NUCLEAZE` k-mer screen and the `BOWTIE2`/`MINIMAP2` alignment indices, so `EXTRACT_VIRAL_READS` screening and alignment results both change on the next index rebuild; no outputs renamed or removed. Adds a `virus-genomes-mask-rrna.stats.txt` BBDuk stats log (#847).
