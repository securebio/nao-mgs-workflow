...
- Compress `SORT_FILE` and `SORT_FASTQ` output with `pigz -1` rather than the default level 6, so all three sort modules and the rest of the pipeline agree. `NUCLEAZE` already writes the largest outputs in RUN at level 1; on a real hits table level 1 is 3.9x faster than level 6 for output 65% larger, and these files are intermediates. Contents are unchanged; the `.gz` bytes differ. (#949)
- Rewrite `SORT_TSV` as a single streaming pipeline on `lib/SortUtils.groovy`, so it uses the same GNU `sort` setup as `SORT_FILE` and `SORT_FASTQ`. The table is no longer decompressed into the work directory twice before sorting, `sort`'s spill moves off the work directory onto local disk with level-1 compression, and task CPU and memory are tiered on input size via `sort_resources` instead of a fixed 1 CPU / 16 GB. Replaces `sort_tsv.py` with a shell pipeline and adds `gawk` to the `coreutils` container. Sorted contents are unchanged for well-formed input, but the `.gz` bytes differ because output compression moves from single-threaded `gzip -9` to `pigz -1`, matching the treatment of other pipeline intermediates. Rows are now passed through byte for byte: the Python implementation stripped leading and trailing whitespace from the first data row only, which both altered that row and moved it in the sort order, and it normalised CRLF line endings to LF. (#949)
