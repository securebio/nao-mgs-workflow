...
- Treat a Virus-Host DB host of root (taxid 1) or blank as absent rather than as non-infecting, so such taxa reach `UNRESOLVED` and are resolved from the taxonomy instead of being marked 0. Changes the annotated virus DB on next index rebuild; no taxon enters or leaves infection status 1, so the surveilled genome set is unchanged. (#PRNUM)
