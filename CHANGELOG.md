...
- Add `prepare-release` skill (`.claude/skills/prepare-release/`) for cutting the release PR into dev described in `docs/developer.md` § "New releases" step 2. Reads dev's accumulated `-dev` CHANGELOG bullets, classifies the overall bump level per `docs/versioning.md`, rewrites the bullets as a polished release note mirroring the v3.2.1.0 / v3.2.1.3 grouped structure, updates `pyproject.toml` + the CHANGELOG heading, and opens a `release/<handle>/<version>` branch. Lighter `version-bump` agent remains the right tool for mid-stream bumps.
    - Extend `bin/check_version.py` release-branch detection to recognize `coding-agent/release/<version>` in addition to `release/<handle>/<version>`. Required because the `securebio-coding-agent` GitHub App can only push under `coding-agent/*`; without this, agent-authored release PRs failed the dev-suffix check.
