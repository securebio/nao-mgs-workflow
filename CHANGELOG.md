...
- Skip generating minimap2 outputs that no downstream process consumes, cutting overall pipeline cost by ~10% in cpu-hours. `MINIMAP2` and `MINIMAP2_SPLIT_INDEX` gain `keep_sam`, `keep_mapped` and `keep_unmapped` flags (all defaulting to `true`, following the `keep_match`/`keep_nomatch` pattern in `NUCLEAZE`); when a flag is false, that branch's FIFO, `samtools` pass and `pigz` compressor are never created. The human and contaminant alignments in `extractViralReadsONT` now emit only unmapped reads, the viral alignment only the SAM and mapped reads, and the ribosomal alignment in `profile` drops the SAM. None of the skipped outputs are published, so results and schemas are unchanged.
