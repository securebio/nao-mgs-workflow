...
- Triage urllib3 2.6.3 vulnerabilities (CVE-2026-44431 cross-origin header leak, CVE-2026-44432 decompression-bomb safeguard bypass) flagged by Trivy on `blast`, `bowtie2_samtools`, `kraken2`, `minimap2_samtools`, and `multiqc` containers. Pin `urllib3>=2.7.0` in `containers/multiqc.yml` (transitive via `requests`; conda solver permits the upgrade) so the next multiqc-image rebuild picks up the fix. For the four awscli-bearing containers, conda-forge `awscli` pins `urllib3<=2.6.3` through the current latest 2.34.37, so the upgrade is blocked upstream — ignore both CVEs with reasoning tied to the actual usage (`aws s3 sync` against trusted NAO buckets, no cross-origin redirects, no Brotli) and re-eval trigger (awscli feedstock relaxing its urllib3 cap).
