...
- Add `bin/benchmark_index.py` and the paired `benchmark-index` agent skill for vetting a candidate `s3://nao-mgs-index/<DATE>` build before promoting it. The script diffs two index roots and writes per-DB size deltas, per-host infection-status transitions, categorized lost / gained genome IDs, a params diff, and a top-line `summary.md`; the skill turns those into a structured ship-or-regen `REVIEW.md`.
