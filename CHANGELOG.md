...
- Make `DOWNLOAD_VIRAL_GENOMES` retry the dehydrated `datasets download` step (not just rehydrate) with exponential backoff via a shared `retry()` helper. Transient NCBI HTTP/2 stream errors (`INTERNAL_ERROR; received from peer`) on the download step previously aborted the task, and Nextflow's immediate task retry couldn't ride out the server-side error window; spaced in-script attempts can.
- The INDEX workflow now publishes `virus-genome-metadata-raw.tsv.gz`, the full set of viral assemblies enumerated by `datasets summary` (columns: `assembly_accession`, `taxid`, `organism_name`, `source_database`, `assembly_status`, `release_date`), captured before the host-infection and assembly-status filters drop genomes. This records each assembly's build-time taxid and release date so downstream tooling (e.g. index benchmarking) can attribute why a genome was or wasn't included — including telling genomes genuinely deposited since the previous build from pre-existing ones surfaced by a config change — without live NCBI lookups. New published output only; no existing output changed and no RUN/DOWNSTREAM consumer reads it.
