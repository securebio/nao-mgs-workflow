...
- `FASTP`: replace single-threaded `gzip -c` output (and the empty-file fixups) with `pigz -p ${task.cpus} -1`, so fastp's 8-core allocation is no longer bottlenecked at one core for compression. Adds `pigz=2.8` to `containers/fastp.yml`.
