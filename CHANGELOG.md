...
- Speed up `CONCATENATE_GENOME_FASTA` by parallel-fetching staged genomes with `xargs -P` and bumping label from `single` to `xsmall`. Also adds the process to the `/scratch` staging list in `configs/profiles.config`.
