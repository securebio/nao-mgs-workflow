...
- Add a unified pipeline benchmarking architecture under `.claude/`. Two subagents (`bench-workflow-batch` and `bench-module-local`) own the actual execution for cohort-on-Batch and single-module-on-sandbox modes respectively, calling shared scripts at `.claude/scripts/parse_bench_trace.py` and `.claude/scripts/bench_output_equality.py` for trace aggregation and S3 result equality. The skill at `.claude/skills/bench/` is the dispatcher: mode selection + how to integrate agent output into a PR description. Whitelists `.claude/skills/` and `.claude/scripts/` in `.gitignore`, extends mypy CI to cover `.claude/scripts/`.
