...
- Set `wave.tokens.cache.maxDuration = '24h'` and `wave.retryPolicy.maxAttempts = 10`, so Wave stops renaming the same image mid-run and a Wave `429` gets ~3 minutes of retry instead of ~6 seconds. Adds `tests/workflows/wave.nf.test`, which asserts both values reach the Wave client: `wave.tokens.cache.maxDuration` is undocumented, and Nextflow silently ignores config keys it doesn't recognise.
