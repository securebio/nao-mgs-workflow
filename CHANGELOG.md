...
- Speed up `CONCATENATE_GENOME_FASTA` and `ADD_GENBANK_GENOME_IDS` by parallel-fetching staged genomes with `xargs -P` and bumping label from `single` to `xsmall`. Also adds both processes to the `/scratch` staging list in `configs/profiles.config`. Inline Python in `ADD_GENBANK_GENOME_IDS` is refactored into a dedicated `add_genbank_genome_ids.py` script (with pytest coverage).
- Speed up `DOWNLOAD_VIRAL_GENOMES` by staging on local `/scratch` (configured per-process in `configs/profiles.config`) and replacing per-file `find -exec mv` with batched `xargs mv`.
