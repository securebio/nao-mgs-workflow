...
- Pin msgpack-python to 1.2.1 and setuptools to 78.1.1 in the pysam_biopython container to clear two HIGH CVEs surfaced by the container rebuild, and ignore three util-linux `mount(8)` CVEs that the pipeline never reaches and for which no base-image fix is published.
