...
- Configure Ruff and apply behaviour-changing fixes flagged by Ruff lint rules (preparation for the broader Ruff lint sweep landing as a follow-up PR). Specifically:
    - Configure `[tool.ruff]` in `pyproject.toml` for compatibility with `ruff format`: drop `COM` (trailing-comma) from `lint.select` (the formatter already manages trailing commas), and exclude `.nf-test`, `tmp`, `.venv`, `post-processing/deps`, and `test-data/tiny-index/output/logging` to prevent stale `[tool.ruff]` blocks from being picked up by hierarchical config discovery.
    - Fix all ten `B904` violations by chaining (`raise ... from e`) the original exception in the new one in every case. This adds the `__cause__` traceback chain for these errors, slightly changing how they print.
    - Fix a latent bug in `bin/prepare_tiny_test_data.py::upload_to_s3`: `s3_uri.lstrip("s3://")` (which treats the argument as a character set, not a literal prefix) replaced with `removeprefix("s3://")` (`B005`). Add `bin/test_prepare_tiny_test_data.py` regression test covering the standard URI cases and the regression cases (URIs whose path starts with characters in `{s,3,:,/}`).
    - Audit and switch all eight bare `zip()` call sites to `strict=True`. Each site zips iterables that are guaranteed equal-length by construction (paired R1/R2 FASTQ records, columns of a single DataFrame, statically-equal lists, etc.), so this codifies the invariant: a future bug that breaks it now raises `ValueError` immediately instead of silently truncating.
    - Mention `ruff format` in the `docs/developer.md` "Running Python tools" block so contributors run both the linter and the formatter.
