...
- Update the SSU ribosomal reference from SILVA 138.2 to SILVA 144 (`SSURef_NR99_trunc`), which changes RUN's ribosomal/non-ribosomal split. LSU stays at SILVA 138.2, as SILVA 144 has no LSU release. Cap BBDuk's Java heap at 75% of task memory, since the larger reference otherwise lets uncollected garbage approach the task's memory limit. (#1013)
