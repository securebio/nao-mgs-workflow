...
- Replace the BBDuk-based viral k-mer pre-screen in `EXTRACT_VIRAL_READS_SHORT` with [Nucleaze](https://github.com/jackdougle/nucleaze), which consumes a pre-built binary k-mer index now produced by INDEX. RUN snapshots are byte-identical to the BBDuk implementation; the change is purely a performance/I-O improvement (the k-mer index is built once at INDEX time and reused across runs). The BBDuk-based ribosomal screen in `PROFILE` is unchanged — Nucleaze does not yet support a fraction-based threshold equivalent to BBDuk's `minkmerfraction`. Bumps `pipeline-min-index-version` to `3.2.2.0`: pipelines on this version require an index containing `virus-genomes-masked.nucleaze.bin`.
    ...
    - RUN reads `nucleaze_k` from the index's `input/index-params.json` rather than from `params.nucleaze_k`, removing a silent foot-gun where the INDEX-time `k` and the RUN-time `k` could drift apart.
    - The `NUCLEAZE` process now takes `keep_match` and `keep_nomatch` boolean flags (both default `true`). When a side is dropped the process writes that fraction to `/dev/null` directly and skips the pigz worker, eliminating compression cost for output the caller doesn't consume. `EXTRACT_VIRAL_READS_SHORT` passes `keep_nomatch: false`, since the viral subworkflow only uses the match fraction.
