...
- Add a `FILTER_METADATA_TO_FASTA` module and call it at the end of `MAKE_VIRUS_GENOME_DB`, so the published `virus-genome-metadata-gid.tsv.gz` carries exactly one row per sequence in `virus-genomes-masked.fasta.gz` instead of a row per (assembly, sequence) pair downloaded: rows describing sequences absent from the FASTA are dropped, and rows sharing a `genome_id` are reconciled to the copy the FASTA carries. INDEX now fails in this step, rather than publishing unreconciled metadata, if a published sequence has no metadata row, if rows sharing a `genome_id` disagree on a per-sequence column, or if the genome FASTA repeats a sequence ID. `PREPARE_VIRAL_METADATA`'s output is renamed to `virus-genome-metadata-gid-unfiltered.tsv.gz` and stays internal to the subworkflow.
