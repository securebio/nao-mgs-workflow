...
- Add `bench-module-local` subagent (`.claude/agents/`, with `.config` fixture and `.claude/skills/bench-module/` dispatcher) for A/B-benching a single Nextflow process across two branches via local Docker. Uses signature-driven construction — identity, inline transform, synthesis, or upstream chain — and aggregates traces via the new `.claude/scripts/parse_bench_trace.py` (canonical metric definitions per `.claude/benchmarking.md`). `CLAUDE.md` and `.gitignore` updated; mypy CI extended to `.claude/scripts/`.
- Add `bench-workflow-batch` subagent (`.claude/agents/`) and `bench-workflow` skill (`.claude/skills/`) for full-pipeline A/B benches on AWS Batch via `bin/chain_workflows.py`. Skill fans out one single-branch agent per branch in parallel, then aggregates traces via `parse_bench_trace.py` and verifies per-sample output equality via the new `.claude/scripts/bench_output_equality.py` (decompress + sort + md5 per file, with known-noise classification for Kraken HLL and FastQC duplicates baked in). Type-checked, 28 pytest cases.
- Add top-level `bench` skill (`.claude/skills/bench/`) as the canonical entry point for benchmarking a perf PR. Routes to `bench-module` and/or `bench-workflow` based on the claim shape, then walks the agent through composing the bench section of the PR description per `.claude/pr-examples/pipeline-bench.md` — including the cohort context, interpretation paragraphs, critical-path framing, and scope caveats that the underlying scripts don't produce on their own. `CLAUDE.md` "Benchmarking conventions" updated to point at the top-level skill as the recommended entry point.
