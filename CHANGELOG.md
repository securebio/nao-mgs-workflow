...
- INDEX now publishes `virus-genome-metadata-raw.tsv.gz`: every enumerated viral assembly (columns `assembly_accession`, `taxid`, `organism_name`, `source_database`, `assembly_status`, `release_date`) before the host-infection and assembly-status filters, so tooling (e.g. index benchmarking) can attribute why a genome was or wasn't included. New output only; nothing existing changed.
