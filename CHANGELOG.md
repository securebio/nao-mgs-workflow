...
- Swap `zcat` for `rapidgzip --count-lines` in `COUNT_READS` for ~3-4× faster single-thread gzip inflate; resource allocation unchanged at `single` (1 cpu). Adds `rapidgzip=0.15.2` to the `coreutils_gzip_gawk` container. Extending pigz/rapidgzip to other decompression hot paths is tracked in #776. (#777)
- Replace single-threaded gzip/zcat with pigz across `EXTRACT_VIRAL_READS_SHORT` (`BBDUK`, `BBDUK_HITS_INTERLEAVE`, `BOWTIE2`, `FASTP`, `SORT_FASTQ`, `SORT_FILE`); pipeline outputs preserve read content but not stream order. Adds `pigz=2.8` to `bbtools`, `bowtie2_samtools`, `fastp`, and `coreutils` containers. (#774)
- Combine `SUBSET_READS_PAIRED_TARGET` and `INTERLEAVE_FASTQ` into a single FIFO-based process that reads each input once and emits interleaved output via `seqtk mergepe`; plumb the upstream `COUNT_READS` TSV through to eliminate the in-process read-counting pass. Bumps `SUBSET_READS_*` allocation from `single` to `xsmall` and adds `pigz=2.8` to the `seqtk` container. (#775)

## Bugfixes

- Fix `samtools view` failure on duplicate sequence IDs in the viral genome FASTA produced by `CONCATENATE_GENOME_FASTA`. NCBI `datasets` sometimes returns superseded ("previous") assembly versions alongside current ones, causing duplicate accessions. (#758)
    - `DOWNLOAD_VIRAL_GENOMES` now emits an `assembly_status` column in the per-taxid metadata TSV, which `FILTER_VIRAL_GENBANK_METADATA` uses to keep only rows with `assembly_status == 'current'` (dropping `previous`, `replaced`, `suppressed`, etc.).
    - `CONCATENATE_GENOME_FASTA` runs `seqkit rmdup --by-name` after concatenation as a defense-in-depth check; the process moves from the `seqtk` container to the existing `seqkit` container.
- Fix `CONCATENATE_GENOME_FASTA` `SIGPIPE` on large genome directories by adding an `|| true` escape path to the `head` call. (#779)
...
- Improve Nextflow version-checking CI: replace the hardcoded `EXCLUDED_VERSIONS` constant in `bin/check_nextflow_version.py` with a `.nextflowignore` config file supporting permanent and time-limited (`exp:YYYY-MM-DD`) ignores, switch target selection to highest-semver-among-non-ignored, and bump the pinned Nextflow version from `25.10.4` to `25.10.5`. (#760)
- Extract the chained `chain_workflows.py` invocation shared by both benchmark workflows into a reusable composite action at `.github/actions/run-benchmark/`, and add a manual `benchmark-on-demand.yml` workflow (`workflow_dispatch` only) that calls it with a `dataset:` choice input. The PR-triggered `benchmark-illumina-100M.yml` and `benchmark-ont-100k.yml` become thin callers of the same action; their behavior on PRs to `main`/`stable` is unchanged. Per-run `--base-dir` is now keyed by `${{ github.run_id }}` so manual and auto-on-PR runs don't clobber each other's S3 outputs. (#773)
- Register `.github/workflows/build-containers.yml` as a no-op stub so feature branches can dispatch a real implementation via `workflow_dispatch` once the stub reaches `main`. Companion PR #792 carries the real build-and-push implementation. Also defer Nextflow `26.04.1` in `.nextflowignore` (same expiry as the existing `26.04.0` defer, 2026-06-01). (#793)
- Add Trivy `.trivyignore` waivers for HIGH CVEs with no current Debian fix and no practical pipeline exposure: CVE-2026-4878 (libcap2); CVE-2026-33845, CVE-2026-33846, CVE-2026-3833, CVE-2026-42010, CVE-2026-42011 (libgnutls30 TLS-PSK); CVE-2026-42311 (Pillow in multiqc); GHSA-82j2-j2ch-gfr8 (rustls-webpki in multiqc); and 5 Go stdlib 1.23.4 CVEs (CVE-2026-33811, -33814, -39820, -39836, -42499) in the `ncbi_datasets` container, pending an NCBI rebuild on Go ~= 1.25.10 / 1.26.3. (#755, #759, #762, #774, #778)

## Coding agents

- Add `.claude/pr-examples/` with worked examples of well-structured PR descriptions for this repo, and a `CLAUDE.md` reference pointing contributors at it. (#767)
- Add `prepare-release` skill (`.claude/skills/prepare-release/`) for cutting a release PR into dev: reads accumulated `-dev` CHANGELOG bullets, classifies the overall bump level per `docs/versioning.md`, rewrites them into a polished release note, updates `pyproject.toml` + the CHANGELOG heading, and opens a `release/<handle>/<version>` PR. Extends `bin/check_version.py` release-branch detection to recognize the `coding-agent/release/<version>` form required when the `securebio-coding-agent` GitHub App pushes the branch. (#794)
