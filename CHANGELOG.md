...
- Set `wave.tokens.cache.maxDuration = '24h'` and `wave.retryPolicy.maxAttempts = 10` to reduce transient Wave infrastructure failures. Adds new test and CI workflow to check the private `wave.tokens.cache.maxDuration` still reaches the Wave client.
