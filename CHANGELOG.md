...
- `benchmark-downstream`: emit `viral_validation_decomposition.tsv`, the deterministic which-side-moved check for a BLAST-agreement drop. For reads validated on both sides it splits agreement losses into those with a changed aligner call (a reassignment) versus an unchanged aligner call (the BLAST validation target moved, e.g. a taxonomy rename), and names the dominant validation-target shift. Lets the report attribute an agreement drop to the rename-vs-reassignment cause rather than guessing. Developer/agent tooling only.
