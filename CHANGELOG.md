...
- Add `bin/benchmark_index.py` and the paired `benchmark-index` agent skill for vetting a candidate `s3://nao-mgs-index/<DATE>` build before promoting it. The script diffs two index roots and writes per-DB size deltas, per-host infection-status transitions, categorized lost / gained genome IDs, a params diff, and a top-line `summary.md`; the skill turns those into a structured ship-or-regen `REVIEW.md`. Both lost and gained genome IDs are categorized exactly from the target index's `virus-genome-metadata-raw.tsv.gz` (build-time taxid, assembly status, and release date) and the old/new annotated DBs — no live NCBI lookups — keyed on each genome's assigned (leaf) taxon, never a species-rank rollup. Lost: absent from NCBI (suppressed/removed), non-current assembly version, hard-excluded, reassigned to an excluded taxon, infection-status demotion. Gained: newly deposited since the old build, hard-included, new taxon in NCBI taxonomy, infection-status promotion, or pre-existing re-included by a change in the index's inclusion config (the §3.3 findings name the driver from the params diff + source mix — e.g. a GenBank→all source switch surfacing RefSeq). The target index must publish that pre-filter output (with its `release_date` column); the script errors out if it is missing. A §3.4 reports genomes reassigned within the concern set (present in both builds, `species_taxid` changed) as an informational sanity check — coverage is unchanged, but a large fraction would flag a mass re-assignment.
