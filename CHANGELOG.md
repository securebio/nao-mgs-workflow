...
- Add a pipeline benchmarking architecture under `.claude/`. Two single-branch subagents (`bench-module-local` and `bench-workflow-batch`) each bench one branch via local Docker or AWS Batch respectively. Two mode-specific skills (`bench-module` and `bench-workflow`) fan out parallel agent invocations per branch and aggregate via shared scripts (`.claude/scripts/parse_bench_trace.py` for trace summary/comparison, `.claude/scripts/bench_output_equality.py` for S3 result equality with known-noise classification baked in). The top-level `bench` skill dispatches between the two modes and documents PR-description integration. Whitelists `.claude/skills/` and `.claude/scripts/` in `.gitignore`; extends mypy CI to cover `.claude/scripts/`.
