...
- Add a pipeline benchmarking architecture under `.claude/`. The `bench-module-local` agent A/B-benches a single Nextflow process across two branches via local Docker — inspecting the module's input signature, picking the shortest construction path that produces matching-shape inputs from the samplesheet (identity, inline transformation like seqtk mergepe, or at most one upstream Nextflow process), assembling a thin Nextflow entrypoint, and running both branches in worktrees. The `bench-workflow-batch` agent benches one branch via `bin/chain_workflows.py` on AWS Batch and is fanned out in parallel per branch by the `bench-workflow` skill, which then aggregates traces and verifies output equality. Shared scripts at `.claude/scripts/parse_bench_trace.py` and `.claude/scripts/bench_output_equality.py` produce all reported numbers (canonical metric definitions per `.claude/benchmarking.md`; known-noise classification baked in for Kraken HLL and FastQC duplicates). The top-level `bench` skill dispatches between the two modes and documents PR-description integration. Whitelists `.claude/skills/` and `.claude/scripts/` in `.gitignore`; extends mypy CI to cover `.claude/scripts/`.
