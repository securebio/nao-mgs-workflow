...
- Add `.claude/skills/bench/` with a unified pipeline benchmarking procedure. Documents the two real modes (module-level local Nextflow + workflow-level AWS Batch via `chain_workflows.py`), shared metric / output-equality / critical-path conventions, and traps. Whitelists `.claude/skills/` in `.gitignore` so future repo-local skills are tracked.
