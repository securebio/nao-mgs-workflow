...
- Speed up `CONCATENATE_GENOME_FASTA` and `ADD_GENBANK_GENOME_IDS` by parallel-fetching staged genomes with `xargs -P` and bumping label from `single` to `xsmall`. Inline Python in `ADD_GENBANK_GENOME_IDS` is refactored into a dedicated `add_genbank_genome_ids.py` script (with pytest coverage).
