...
- Replace Bracken with Kraken2-derived domain abundance estimates, keyed on stable NCBI taxids rather than rank codes. This avoids the empty `{sample}_bracken.tsv.gz` outputs produced when newer Kraken2 databases encode domains with non-`D` rank codes. The compatibility filename and columns are retained, and the same table is also published as `experimental/{sample}_kraken_domains.tsv.gz`.
