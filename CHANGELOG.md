...
- Add a local module-benchmarking subagent (`.claude/agents/bench-module-local.md`, with its `.config` fixture) for A/B-benching a single Nextflow process across two branches via local Docker. The agent inspects the module's input signature and picks the shortest construction path that produces matching-shape inputs from the samplesheet — identity, inline interleave, or an upstream chain when input content drives performance. Both branches run concurrently in git worktrees; traces aggregate to canonical PR-table format via the new `.claude/scripts/parse_bench_trace.py` (metric definitions per `.claude/benchmarking.md`). The `bench-module` skill (`.claude/skills/bench-module/`) is a thin dispatcher. Whitelists `.claude/scripts/` and `.claude/skills/` in `.gitignore`; extends mypy CI to cover `.claude/scripts/`; references the agent from the "Benchmarking conventions" section of `CLAUDE.md`.
