...
- Tolerate viral taxa with no linked NCBI assemblies in `DOWNLOAD_VIRAL_GENOMES`.
    - The module now detects the `datasets download genome taxon` "no genome data is currently available" error and emits an empty `${taxid}_genomes/` (declared `optional`) plus a header-only `${taxid}_metadata.tsv`, instead of failing the whole `MAKE_VIRUS_GENOME_DB` subworkflow whenever any single child taxon happens to have no assemblies.
    - This covers the case where NCBI publishes a new viral family/genus to the taxonomy before assemblies are linked to it.
...
- Prevent `MASK_FASTQ_READS` from running out of memory on merged ONT libraries larger than ~6 GB of gzipped FASTQ (#737).
    ...
    - Documents the config-scope closure pattern in `docs/developer.md` as a variant of the existing "resources live in `configs/resources.config`" rule, including the `lib/` classpath caveat.
