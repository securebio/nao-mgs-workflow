...
- Filter unknown hosts and unknown taxids out of host-infection overrides before the per-host annotation loop in `annotate_virus_db()`. Prevents an overrides file containing only invalid taxids from bypassing the no-direct-matches short-circuit in `check_infection()` and tripping the `UNRESOLVED` assertion in `mark_descendant_infections`; warns the user with the dropped names/taxids (sample truncated at 10).
