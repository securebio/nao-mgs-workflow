...
- Add a `FILTER_METADATA_TO_FASTA` module that filters a genome metadata TSV down to one row per sequence present in a genome FASTA, dropping rows for absent sequences and reconciling rows that share a `genome_id`. Fails if a sequence in the FASTA has no metadata row, or if rows sharing a `genome_id` disagree on a per-sequence column. Not yet called by any workflow.
- Call `FILTER_METADATA_TO_FASTA` from `MAKE_VIRUS_GENOME_DB`, so the published `virus-genome-metadata-gid.tsv.gz` carries exactly one row per sequence in `virus-genomes-masked.fasta.gz` instead of a row per (assembly, sequence) pair downloaded. INDEX now fails in this step, rather than publishing unreconciled metadata, if a published sequence has no metadata row or if rows sharing a `genome_id` disagree on a per-sequence column. `PREPARE_VIRAL_METADATA`'s output is renamed to `virus-genome-metadata-gid-unfiltered.tsv.gz` and stays internal to the subworkflow.
