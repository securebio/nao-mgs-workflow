...
- Build `bowtie2-build` against libsais in the `bowtie2_samtools` container, and bump bowtie2 to 2.5.5. Index construction switches from bowtie2's blockwise suffix sort to `libsais64_omp`, which is substantially faster and produces byte-identical indexes. Only the `bowtie2-build-{s,l}` binaries are replaced; the aligner is unchanged. Requires a container rebuild to take effect. (#TBD2)
