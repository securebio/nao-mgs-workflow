...
- Key alignment duplicate marking on each mate's unclipped 5′ coordinate and strand, as `samtools markdup` does, rather than on the two mates' alignment start coordinates. Fixes two over-merging bugs: fragments shorter than the read no longer collapse onto a shared start coordinate, and molecules occupying one span in opposite orientations are no longer grouped. DOWNSTREAM now requires short-read RUN output produced at this version or later, since `mark_duplicates` reads the unclipped coordinate columns. (#1006)
