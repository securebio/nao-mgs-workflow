...
- Enable Nextflow static typing via `nextflow.enable.types = true` in `configs/profiles.config`, and document static-typing conventions (record types, records over tuples, no `.out` access, params stay config Maps) in `docs/developer.md`. Additive; existing untyped code keeps working, no behavioral change.
