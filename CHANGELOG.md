...
- Add a local module-benchmarking subagent at `.claude/agents/bench-module-local.md` (with its `.config` fixture) for A/B-benching a single Nextflow process across two branches via local Docker. The agent inspects the module's input signature, picks the shortest construction path that produces matching-shape inputs from the samplesheet (identity, inline transformation, or upstream Nextflow processes as needed), assembles a thin entrypoint, runs both branches in worktrees, and aggregates traces via the new `.claude/scripts/parse_bench_trace.py` (canonical metric definitions per `.claude/benchmarking.md`). Whitelists `.claude/scripts/` in `.gitignore`; extends mypy CI to cover `.claude/scripts/`.
