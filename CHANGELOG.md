...
- Triage two HIGH urllib3 CVEs (CVE-2026-44431 cross-origin header leak, CVE-2026-44432 decompression-bomb bypass) flagged by `scan-containers`. Patched the `multiqc` container by adding an explicit `conda-forge::urllib3>=2.7` pin in `containers/multiqc.yml` (overrides the urllib3 2.6.3 pulled in transitively via `multiqc` → `requests`). Added the same two CVEs to `.trivyignore` for `blast`/`bowtie2_samtools`/`kraken2`/`minimap2_samtools`, where urllib3 is pulled in via `awscli=2.34.9` and every current conda-forge `awscli` build pins `urllib3<=2.6.3`; the awscli usage in those containers is `update_blastdb.pl --source aws` and equivalents, which fetch over HTTPS from known endpoints without a proxy or attacker-controlled compressed streams, so neither CVE is reachable. Re-eval trigger: awscli feedstock relaxes the urllib3 cap.
