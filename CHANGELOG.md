...
- Activate viral sequence-sourcing (COMP-2466): `MAKE_VIRUS_GENOME_DB` gains a `virus_source` param (`assembly` | `sequence` | `both`, default `assembly`). `both` unions the NCBI genome-assembly branch with a new NCBI Virus / nuccore sequence branch — recovering recent non-influenza viral genomes that NCBI no longer publishes as assemblies — excluding influenza from the sequence branch (kept on the assembly branch) and collapsing cross-source duplicates (FASTA via `seqkit rmdup`, metadata by `genome_id`). New params `datasets_summary_seq_extra_args` / `datasets_download_seq_extra_args` / `sequence_exclude_taxid` tune the sequence branch. Default `assembly` reproduces prior behavior exactly.
...
- Add a sequence-based download path to `DOWNLOAD_VIRAL_GENOMES`, selectable via a new `source_type` input (mirrors `ENUMERATE_VIRAL_ACCESSIONS`). The sequence path downloads NCBI Virus / nuccore records (`datasets download virus genome accession`, no dehydrate/rehydrate) into the same combined-FASTA + `assembly_accession`→`genome_id` map contract, with an identity map (`genome_id == accession`) since each record is its own genome. Dormant — the subworkflow still uses the assembly path; groundwork for COMP-2466.
