...
- Scale `MARK_ALIGNMENT_DUPLICATES` and `VSEARCH_CLUSTER` memory by input size via per-label tier closures to address OOM issues on large samples. Thresholds were set from measured `peak_rss` on synthetic inputs built from real viral hits (MARK ≈ 4.2× input size; VSEARCH ≈ 13× the largest per-species input).
