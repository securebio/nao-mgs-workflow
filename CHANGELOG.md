...
- Add `.github/workflows/build-containers.yml`, a `workflow_dispatch`-only CI workflow that rebuilds containers from their `containers/*.yml` specs and pushes the resulting images to ECR, using the CI's existing OIDC role for the push. Triggered manually by a maintainer or programmatically by the coding-agent GitHub App via `gh workflow run`; the dispatching caller then downloads the updated `configs/containers.config` as an artifact and commits it to the branch. Inputs default to building only specs that differ from `origin/dev`; `all` or a comma-separated explicit list also supported. This closes the rebuild gap that the `triage-trivy` skill noted: an agent that patches a `containers/*.yml` to address a CVE can now trigger the image build itself rather than waiting on a maintainer.
