...
- Pin `pandas==2.3.3`, `pandas-stubs~=2.3.0`, `biopython==1.85`, `jsonschema==4.25.0`, and `pysam==0.23.3` so the dev/CI environment matches the conda pins used by the pipeline containers (`containers/*.yml`). The previous `>=` floors let CI float above the versions production runs (e.g. pandas to a 3.0 major), type-checking and testing against APIs the containers never use.
...
- Ignore eight HIGH/CRITICAL CVEs surfaced by `scan-containers` after the Debian trixie base move: perl-base CVE-2026-42496 / CVE-2026-8376 / CVE-2026-42497 / CVE-2026-48962 / CVE-2026-9538 (unreachable; perl only runs `update_blastdb.pl` on trusted NCBI DBs) and Go-stdlib CVE-2026-39823 / CVE-2026-39825 / CVE-2026-39826 in the `ncbi_datasets` vendored binaries (server-side bugs unreachable in client-only CLIs).
