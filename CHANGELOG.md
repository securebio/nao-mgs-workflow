...
- Move `BOWTIE2_INDEX` from the `max` label to a new `bowtie2_index_resources` label (32 cpus, memory tiered on reference size: 64/128/256 GB). libsais holds the whole suffix array in memory rather than spilling it in blocks, and the contaminant reference does not fit in the 64 GB of `max`; tiering keeps the smaller virus and human builds at 64 GB. (#985)
- Add an optional `build_steps` field to container specs — shell commands run inside the image after the Conda environment is installed — and use it to build `bowtie2-build` against libsais, bumping bowtie2 to 2.5.5. Index construction switches from bowtie2's blockwise suffix sort to `libsais64_omp`, which is substantially faster and produces byte-identical indexes. Only the `bowtie2-build-{s,l}` binaries are replaced; the aligner is unchanged. Requires a container rebuild to take effect. (#983)
- Add `bin/test_build_ecr_container.py`, covering spec validation, hash stability and invalidation, ECR image-existence handling, Dockerfile generation, and container-config rewriting. Tests only; no behaviour change. (#987)
