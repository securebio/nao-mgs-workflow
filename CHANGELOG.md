...
- Add `bin/benchmark_index.py` and the paired `benchmark-index` agent skill for vetting a new `s3://nao-mgs-index/<DATE>` build before promoting it to production. The script takes two index roots (`--old`, `--new`, each `s3://` or local) and emits per-DB size deltas, per-genome-id categorization of lost and gained genomes (`hard_excluded` / `change_in_assigned_taxid` / `species_dropped_from_metadata` / `non_current_genome_version` / `other` on the losses side; `hard_included` / `change_in_assigned_taxid` / `newly_deposited_existing` / `infection_status_change` / `new_species_in_taxonomy` / `other` on the gains side), per-species genome-count deltas, virus-taxonomy DB add/drop, per-host infection-status transition tables annotated with existing-rule coverage via `--repo-root`, reference-DB staleness checks (Kraken2 + SILVA auto-checked; human / taxonomy / VHDB URLs surfaced as "unknown"), and a params diff. Outputs include detailed TSVs (`genomes_lost_categorized.tsv`, `genomes_gained_categorized.tsv`, per-host `infection_status_changes_*.tsv` / `species_transitions_*.tsv`, `species_gained_all_genomes.tsv`, `species_lost_all_genomes.tsv`, `sizes.tsv`, `taxa_added.tsv`, `taxa_removed.tsv`, `params_diff.txt`) and a top-line `summary.md` whose section order mirrors the agent's review template at `.claude/skills/benchmark-index/review-template.md`. The skill runs the script, reads `summary.md`, optionally cross-references VHDB, and fills in `REVIEW.md` framed as a ship-or-regen decision about the candidate index. Pure-function helpers are covered by `bin/test_benchmark_index.py`.
