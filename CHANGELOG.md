...
- Recast `bin/benchmark_index.py` and the `benchmark-index` skill around the reviewer-asserted purpose of the report: a **ship-or-regen decision about this specific index**, not a "fix-it-next-build" log. Recommendations are now framed as ship-as-is vs. regenerate-with-config-change; the "next-build hygiene" confidence level is dropped, the Summary now requires a one-line ship-or-regen headline, and the §1 staleness Findings stop saying "bump in the next build". Lost-gid categories tightened to match the template semantics the reviewer clarified: `infection_status_change` covers same-taxid filter flips across *any* host in `host_taxa_screen` (not just human 0→1); `change_in_assigned_taxid` is gid reassignment to a different taxid that fails the filter (not "taxid retired from NCBI"); new `non_current_genome_version` category captures the `assembly_status == 'current'` filter dropping superseded NCBI assembly versions (was previously hiding in `other` — e.g. 36 SARS-CoV-2 / 21 RSV-A gids in the 20250825 → 20260520 run). Gained-gid categories restructured symmetrically: `change_in_assigned_taxid` is for restructure-destination taxids; `infection_status_change` is for species that were in old taxonomy DB but not old metadata (filter flipped to surveilled); `newly_deposited_existing` now uses old-metadata membership as the "already surveilled" test rather than `infection_status_human == 1`, so the bulk of routine reference-data growth lands in `newly_deposited_existing` rather than `other`. §4 table follows `review-template.md` literally (header `| Host | Promotions | Demotions |`, total counts — not an "actionable" subset). Review template gains a `Non-current genome version` row in §3.1 losses. Tests updated for the new signatures.
- Restructure `bin/benchmark_index.py`'s `summary.md` output and the `benchmark-index` skill to follow a fixed template at `.claude/skills/benchmark-index/review-template.md`. New script-side capabilities: per-genome-id categorization for both losses (`hard_excluded` / `infection_status_demotion` / `species_retired` / `other`) and gains (`hard_included` / `newly_deposited_existing` / `infection_status_promotion` / `species_new` / `other`), surfaced both as new `genomes_lost_categorized.tsv` / `genomes_gained_categorized.tsv` outputs and as the §3.1 totals breakdown in `summary.md`; `species_gained_all_genomes.tsv` complementing the existing lost-species inventory. `summary.md` is reorganized to match the template's section order (Header / Summary / Findings §1–§5 / Recommendations / Appendix A.1–A.14), with script-generated factual `Findings:` bullets per section that the reviewer expands into prose. The skill is rewritten to point at `review-template.md` as the authoritative structure rather than embedding its own. Pytest coverage extended to the two new categorization helpers.
- Add `bin/benchmark_index.py` for comparing two index releases (`--old`, `--new` accepting s3:// or local paths). Emits per-DB size deltas, genome add/drop/per-species counts, virus-taxonomy DB add/drop, per-host infection-status transition tables and per-taxon change lists, and a params diff; writes a top-line `summary.md` flagging shrunk DBs. Pure-function helpers (size diff, genome diff, taxonomy diff, status transitions/changes, params diff) are covered by `bin/test_benchmark_index.py`. Paired with a `.claude/skills/benchmark-index/` agent skill that runs the script, interprets the TSVs (filters infection-status changes to species-rank 1→0 demotions, cross-references upstream Virus-Host-DB to attribute changes to upstream drift vs workflow changes, checks `ref/host-infection-overrides.json` and `viral_taxids_exclude_hard` to avoid duplicate recommendations), and produces a 5-section markdown review report for the maintainer to act on.
