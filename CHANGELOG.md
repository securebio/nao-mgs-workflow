...
- Add `tag "id=<value>"` directives to all `modules/local/` processes for per-task attribution in trace files and Nextflow logs.
    ...
    - Add a static `bin/check_process_tags.py` lint (CI workflow `check-process-tags.yml`) that scans `modules/local/` for any process with a missing or malformed `tag` directive (the literal must match `id=<value>` optionally followed by `,<key>=<value>` components). This statically catches untagged or wrong-format processes that `assertTraceTagsValid` cannot, since the runtime helper only observes processes actually executed by a workflow (orphaned or unreached processes slip through). The runtime helper remains necessary to validate interpolated tag values, which are only known at execution time.
