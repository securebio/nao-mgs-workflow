...
- Combine `SUBSET_READS_PAIRED_TARGET` with the `INTERLEAVE_FASTQ` step into a single FIFO-based pipeline that reads each input file once, samples R1 and R2 in parallel with seqtk, and writes the interleaved output directly. `SUBSET_READS_*` now take an upstream `COUNT_READS` TSV as input (eliminating the in-process read-counting pass), use pigz for parallel (de)compression, and bump from `single` to `xsmall` (4 cpus). The standalone `INTERLEAVE_FASTQ` module is no longer used by the main workflow but is retained for use as a test setup helper by other module tests. Adds `pigz=2.8` to the `seqtk` container. Output FASTQs are byte-identical to the old `SUBSET_PAIRED → INTERLEAVE_FASTQ` chain (same seqtk algorithm, seed, and fraction).
