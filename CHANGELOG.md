...
- Add `FILTER_METADATA_TO_FASTA`, which filters `virus-genome-metadata-gid.tsv.gz` down to the sequences present in the published genome FASTA. The metadata previously described every genome that survived metadata-level filtering, including ones whose sequences header-pattern exclusion had removed, so it overstated the contents of the index it shipped with. The step is anchored on the final FASTA's headers rather than re-deriving which sequences were removed, and fails the build if a published sequence has no metadata row. (#898)
- Move deduplication of the viral genome DB out of `CONCATENATE_GENOME_FASTA` into a new `DEDUP_GENOME_FASTA` step, which runs after `FILTER_GENOME_FASTA`. Behaviour is unchanged — the same `seqkit rmdup --by-name` pass in a new position, which is a no-op for a matcher that only collapses byte-identical headers — but it separates concatenation from deduplication and puts dedup where a stricter matcher can safely replace it. `CONCATENATE_GENOME_FASTA` moves to the `coreutils_gzip_gawk` container and the `single` label, and its output is now a concatenation of per-chunk gzip members rather than a single seqkit-written stream. (#898)
- Deduplicate the viral genome DB by sequence ID and by sequence, replacing `DEDUP_GENOME_FASTA`'s `seqkit rmdup --by-name` pass. Records sharing an ID but differing in description are now collapsed (`--by-name` kept both, leaving the duplicate-reference failure of #758 only partly closed, since `bowtie2-build` and `samtools` key on the ID alone), as are records whose sequence duplicates a retained one on either strand. RefSeq-derived records are ordered ahead of GenBank ones so a RefSeq copy wins dedup against its GenBank twin, keyed on the metadata's `source_database` column rather than an accession prefix. Shrinks the published genome DB and changes the `genome_id` some reads are attributed to. (#898)
- Make `FILTER_GENOME_FASTA`'s header-pattern exclusion header-exact, selecting retained records with `seqkit grep -n` instead of `seqtk subseq`. `seqtk` keys on the sequence ID, so an excluded record was re-admitted whenever a retained record shared its ID and the two differed only in the description the patterns match on. Harmless while `--by-name` kept both copies; with by-ID dedup downstream, the excluded record can become the sole survivor. (#898)
