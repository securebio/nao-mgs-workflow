...
- Replace clustering with downsampling in DOWNSTREAM's post-hoc viral validation: each per-species partition is now downsampled to at most `params.validation_n_sample` reads by a deterministic bottom-N hash sample on `seq_id`, those reads are BLASTed, and every hit carries its own validation result rather than one inherited from a cluster representative. This changes the schema and contents of `validation_hits.tsv.gz`.
    ...
    - Replace the `validation_cluster_identity` and `validation_n_clusters` parameters with `validation_n_sample` (default `20` for short reads, `1000000` for ONT), and drop the injected `cluster_min_len`. The short-read default is set at parity with the number of reads the cluster-exemplar approach actually aligned, so BLAST cost is unchanged; because propagation is gone, most reads are now labelled `not_sampled` rather than inheriting a representative's verdict (in one 767,964-read production group, ~81% propagated values become 280 first-hand ones covering the same species).
    - Remove the VSEARCH clustering step, which in a 507-group DOWNSTREAM run on 3.2.2 consumed 1158 cpu-hours (34% of the workflow) with an extreme tail — a single task ran 2 d 6 h wall / 865 cpu-hours.
    - Delete the `clusterViralAssignments`, `propagateValidationInformation`, and `validateClusterRepresentatives` subworkflows and the `vsearch`, `processVsearchClusterOutput`, and `downsampleFastnById` modules; add the `sampleTsvByHash` and `annotateValidationStatus` modules and the `sampleViralAssignments` and `validateSampledReads` subworkflows. Drop the `vsearch` container (label, spec, and `vsearch_resources` labels) and the now-uncalled `ADD_SAMPLE_COLUMN_LIST` process, and stop emitting FASTQ from `SPLIT_VIRAL_TSV_BY_SELECTED_TAXID` (extraction now happens after downsampling, so only reads being validated are converted).
    - The `rust-tools` `process_vsearch_cluster_output` crate is now unreachable but is left in place; removing it changes the `rust-tools` image and its version wiring, so it is deferred to a follow-up.
