...
- `SUBSET_READS_PAIRED_TARGET` now reads each input FASTQ once, samples R1 and R2 in parallel through FIFOs with seqtk, and merges them into a single interleaved output with `seqtk mergepe`. `SUBSET_READS_*` take an upstream `COUNT_READS` TSV as input (no in-process read-counting pass), use pigz for parallel (de)compression with cpus split between the two parallel input pipelines, and bump from `single` to `xsmall` (4 cpus). The standalone `INTERLEAVE_FASTQ` module is no longer wired into the workflow but is retained as a test setup helper used by other module tests. Adds `pigz=2.8` to the `seqtk` container. Output FASTQs are byte-identical to the previous `SUBSET_PAIRED → INTERLEAVE_FASTQ` chain (same seqtk algorithm, seed, and fraction).
