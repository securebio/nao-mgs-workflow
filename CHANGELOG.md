...
- Write Bracken's header rather than an empty file when a sample has nothing for Bracken to estimate (no reads, or none classified at the estimation level), so `{sample}_bracken.tsv.gz` and `{group}_bracken.tsv.gz` are header-only instead of 0 bytes. Regenerate the DOWNSTREAM empty-input fixtures from real RUN output for a 0-read sample, and check that RUN's empty-input test publishes no headerless table. (#1023)
