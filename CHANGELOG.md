...
- Add `bin/benchmark_index.py` and the paired `benchmark-index` agent skill for vetting a candidate `s3://nao-mgs-index/<DATE>` build before promoting it. The script diffs two index roots and writes per-DB size deltas, per-host infection-status transitions, categorized lost / gained genome IDs, a params diff, and a top-line `summary.md`; the skill turns those into a structured ship-or-regen `REVIEW.md`. Lost genome IDs are categorized exactly by recovering each genome's build-time taxid and assembly status from the target index's `virus-genome-metadata-raw.tsv.gz` — absent from NCBI (suppressed/removed), non-current assembly version, hard-excluded, reassigned to an excluded taxon, or infection-status demotion — with no live NCBI lookups; when that pre-filter output is absent (older index builds) it falls back to a clearly-flagged species-rank approximation.
