...
- Refactor the `WRITE_SENTINEL_RUN` and `WRITE_SENTINEL_DOWNSTREAM` modules into a single parametrized `WRITE_SENTINEL` module (`modules/local/writeSentinel`). The caller now supplies the `expected-outputs-*` key(s), the wildcard and its values, the per-instance tag id / marker filename, and a timestamp field-name prefix, so one module serves both the whole-run and per-group sentinels. `lib/SentinelUtils.groovy` is now loaded explicitly from `params_map.sentinel_utils_path` rather than via Nextflow `lib/` autoload. Pure internal refactor: published outputs, output schema, and the sentinel JSON field names (`runStartedAt`/`runCompletedAt`, `downstreamStartedAt`/`downstreamCompletedAt`) are unchanged.
