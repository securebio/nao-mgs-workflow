...
1. **Identify affected outputs:** Determine the PR's base branch via `gh pr view --json baseRefName -q .baseRefName` (or fall back to `dev` if no PR exists). Run `git diff <base> --name-only` and look for changes to `schemas/`, `modules/`, `subworkflows/`, `workflows/`, or `expected-outputs-*` in `pyproject.toml`.
...
3. **Determine version bump requirements:** Apply the rules from `docs/versioning.md` — schema field changes beyond title/description require a schema (2nd-number) bump; expected-outputs list changes require a schema (2nd-number) bump.
...
- When in doubt about whether a change requires a schema (2nd-number) bump, flag it for human review
