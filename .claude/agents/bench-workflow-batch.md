...
`OUT_DIR` must be absolute (relative paths break when `chain_workflows.py` `cd`s into the worktree) and uniquely random across concurrent invocations (timestamp+PID can collide under heavy parallelism). Use `mktemp -d`:

```bash
OUT_DIR="$(mktemp -d "$PWD/tmp/bench-workflow-XXXXXXXX")"
