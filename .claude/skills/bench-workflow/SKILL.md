...
### 0. Pick a local run-dir for output artifacts

```bash
RUN_DIR="$(mktemp -d "$PWD/tmp/bench-workflow-XXXXXXXX")"
```

Use cwd-relative `./tmp` (not `/tmp`). All per-run artifacts (per-branch trace files, summary md/json, output-equality report) will live under this dir.

For `scratch_base` (S3 prefix): the sandbox default is `s3://sb-det-agent-scratch/bench/` (28-day TTL, agent-default scratch per the sandbox provisioning).
