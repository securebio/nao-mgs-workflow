...
- **Summary**: Follow the template literally — a concise bullet list of top-level findings, then a `**Recommendations:**` sub-list of candidate changes. **Concise** is load-bearing: aim for ≤5 bullets in the top list, ≤5 in the Recommendations sub-list. Each bullet is one short sentence; no nested sub-bullets, no headlines, no "Decision:" framing — the script/skill is not making decisions, the reviewer is. The Recommendations sub-list mirrors the Recommendations section: each bullet is one candidate change (e.g. "Add `38018` to `viral_taxids_exclude_hard`"), not a kind-of-action label. Findings sections below carry the detail; the Summary is a glance-able overview, not a recap. Don't link out to other files.
...
- **Recommendations**: an ordered list of **candidate changes to consider applying before shipping the index**. Each entry is exactly one specific change with a confidence level. Format: `**[specific change]** (high/medium/low)` followed by a 1–2 line justification.
  - Each entry is ONE concrete change — e.g. "Add `38018` to `viral_taxids_exclude_hard`", "Bump `kraken_db` to `k2_standard_20260226.tar.gz`", "Widen the `3047395` *Orthoflavivirus banziense* override to include `primate`". Show the literal config diff when small.
  - Confidence = how sure you are this change is worth making. `high` = clear evidence; `medium` = some uncertainty; `low` = needs a scientist's call.
  - The reviewer picks which to apply — typically folding multiple into a single regen, since regen cost is per-build (~2 hours), not per-fix. Don't pre-decide which to keep; list everything plausibly worth doing and let the reviewer pick.
  - **What does NOT belong here**: "ship as-is" (the default; absence of a recommendation is implicit consent); coordination notes (e.g. "ensure RUN deployments are at ≥ 3.2.2.0" — surface in §5 Findings, not here); scientist-investigation prompts that don't propose a specific change (e.g. "scientist should look at Turlock virus" — either propose the change with `low` confidence, or just note it in §4 Findings). **Never recommend "fix in the next build"** — if a fix is warranted, the candidate change is to regenerate this index with it.
