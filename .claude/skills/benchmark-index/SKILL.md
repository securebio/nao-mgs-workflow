...
- `no_release_date`: the assembly has no `release_date` in the raw metadata, so the newly-deposited-vs-pre-existing split can't be made. The rest of the gained tree is total, so this is the only undecidable case — named explicitly rather than lumped into a catch-all. Should be ~0. (Unlike the lost side, the gained tree has no `other` bucket: every gained genome resolves to one of the reasons above.)
