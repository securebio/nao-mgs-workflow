...
- **Summary**: Follow the template literally — a concise bullet list of top-level findings, then a `**Recommendations:**` sub-list. **Concise** is load-bearing: aim for ≤5 bullets in the top list and ≤5 in the Recommendations sub-list. Each bullet is one short sentence; no nested sub-bullets, no headlines, no "Decision:" framing — the script/skill is not making decisions, the reviewer is. Findings sections below carry the detail; the Summary is a glance-able overview, not a recap. Don't link out to other files; the report stands alone.
