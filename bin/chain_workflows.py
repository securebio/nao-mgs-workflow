...
def resolve_samplesheet_path(
    samplesheet: str, run_launch_dir: Path, repo_root: Path
) -> Path:
    """
    ...
    if samplesheet.startswith("s3://"):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to download samplesheet from S3: {e}") from e
...
def generate_downstream_input(
    downstream_launch_dir: Path,
...
def main() -> None:
    ...
    skip_index = args.skip_index or args.ref_dir is not None
    if not skip_index:
        execute_nextflow(
