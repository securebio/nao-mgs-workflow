...
- Nextflow: size any gather over task outputs with `groupKey(key, n)` and mark every gathering operator with `// check_fan_in: <reason>`, since ignored failures would otherwise pass on partial input (`bin/check_fan_in.py`)
...
Before promoting `dev` to `main`, use the **`benchmark-downstream`** skill (`.claude/skills/benchmark-downstream/`) to diff the DOWNSTREAM output of the two runs and flag regressions for human review. It runs `bin/compare_downstream_runs.py` (munging/IO) with the per-metric calculations in `bin/downstream_metrics.py`, comparing both platforms across four focuses: viral assignments (read-level lost/gained/reassigned joined on `(group, sample, seq_id)`, taxonomic-distance buckets, family/order clade-share breakdown, BLAST-validation agreement, vertebrate-status flips), kraken abundances (Bray-Curtis + top movers per ribosomal set), QC metrics, and a schema-driven file/column overview. It writes per-focus TSVs plus a consolidated `flags.tsv` (fixed thresholds); you fill in `review-template.md` to produce `REVIEW.md`. This is a holistic release diff (code + index + QC differ at once): it surfaces and flags differences for a human and renders no verdict. Mechanism hypotheses from cheap cross-checks (e.g. checking `vertebrate_status_flips.tsv` when a clade drops out of the viral counts) are folded into the finding they concern as a `**Likely mechanism:**` note tagged with one of three evidence-anchored confidence levels (Strongly supported / Consistent / Speculative; never a verdict), rather than living in a separate section. The generated `REVIEW.md` and extracted tables are AWS-derived data and must not be committed.
