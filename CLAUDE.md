...
Before promoting `dev` to `main`, use the **`benchmark-downstream`** skill (`.claude/skills/benchmark-downstream/`) to compare candidate and reference DOWNSTREAM outputs. It writes metric TSVs, fixed-threshold flags, and a concise human review covering viral assignments, Kraken abundances, QC, and output completeness without rendering a verdict. Generated reports and tables are AWS-derived data and must not be committed.
