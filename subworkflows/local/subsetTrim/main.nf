...
include { SUBSET_AND_INTERLEAVE_PAIRED_TARGET as SUBSET_INTERLEAVE_PAIRED } from "../../../modules/local/subsetReads"
...
workflow SUBSET_TRIM {
    ...
    main:
        ...
        // For paired-end, SUBSET_INTERLEAVE_PAIRED also produces interleaved
        // output in a single streamed pass, skipping the SUBSET -> gzip ->
        // zcat -> INTERLEAVE round-trip.
        subset_ch_single = SUBSET_SINGLE(reads_ch_single, params_map.n_reads_profile, params_map.random_seed).output
        inter_ch_paired = SUBSET_INTERLEAVE_PAIRED(reads_ch_paired, params_map.n_reads_profile, params_map.random_seed).output
        inter_ch = subset_ch_single.mix(inter_ch_paired)
        // Read cleaning
