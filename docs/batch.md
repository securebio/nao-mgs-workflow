...
When using Spot instances, AWS can reclaim your instances mid-job. By default, the pipeline's retry logic (`process.errorStrategy = { task.attempt <= task.maxRetries ? 'retry' : 'ignore' }`, `process.maxRetries = 1`) will resubmit failed jobs to the same queue. If the queue uses Spot instances, retries may also be reclaimed during a capacity shortage.
