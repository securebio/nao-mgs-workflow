...
- `params.sequence_exclude_taxid` [str]: Root taxid of a clade dropped from the sequence branch (all its descendants are excluded, computed from the taxonomy `nodes.dmp`). Default: `"11308"` (Orthomyxoviridae / influenza), which stays on the assembly branch because NCBI still publishes grouped flu assemblies; including it in the sequence branch would re-add thousands of ungrouped flu segments. Only used when `virus_source` is `"sequence"` or `"both"`. Note: with `virus_source="sequence"` (assembly branch absent) the excluded clade is dropped from the DB entirely — a sequence-only DB contains no influenza.
