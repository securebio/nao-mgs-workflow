...
- `params.viral_accession_chunk_size` [int]: Max accessions per parallel `DOWNLOAD_VIRAL_GENOMES` task. Default: `10000`. Bounds the long tail of any single shard. Avoid values above ~50000 — `datasets download genome accession --inputfile` rate-limits and per-task wallclock grows quickly past that point.
