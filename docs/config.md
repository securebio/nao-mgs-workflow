...
- `params.blast_db_name` [str]: The BLAST database to download for optional validation of taxonomic assignments — either an `update_blastdb.pl` name (e.g. `core_nt`) or an `http(s)` `.tar.gz` URL (used for CI tests). INDEX publishes it under a fixed `results/blast-db/` directory with a `blast-db` alias.
