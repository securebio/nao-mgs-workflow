...
- `params.download_virus_taxid` [str]: Root taxid whose subtree is partitioned into shards for parallel genome downloads. Defaults to `params.virus_taxid` if empty. Override in test configs to download a smaller subset (e.g., `"2847173"` for Hepatitis D virus 1).
- `params.virus_taxid_max_partition_size` [int]: Maximum subtree size (taxa count) per genome-download shard. The subtree of `params.download_virus_taxid` is partitioned top-down into the shallowest set of subtrees whose sizes do not exceed this value. Lower values yield more, smaller `DOWNLOAD_VIRAL_GENOMES` jobs; the default (`50000`) keeps the largest shard well below sizes empirically known to fail NCBI's dehydrated-zip download stream.
