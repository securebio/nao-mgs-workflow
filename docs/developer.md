...
3. Run `bin/build_wave_container.py PATH_TO_YAML_FILE` to initiate the container build on Wave and update `configs.containers.config` with the container path returned by the script.
