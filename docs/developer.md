...
- Process conventions (see `modules/local/lcaTsv/main.nf` for an example of a well-written process that follows these conventions):
    - All processes should have a label specifying needed resources (e.g. `label "small"`). Resources are then specified in `configs/resources.config`.
        ...
        - The `cpus` directive may be a closure too. Besides buying parallelism, a larger reservation limits how many copies of a process pack onto one host, which is the only way to bound a process's demand on host-local disk: AWS Batch on EC2 schedules on cpu and memory only and ignores Nextflow's `disk` directive (it is Fargate-only). See `sort_file_resources` for an example.
        - A closure over an input variable must use that process's own input name, so a shared closure cannot be reused across processes whose inputs are named differently. Note also that `file` shadows Nextflow's `file()` function and should not be used as an input name in a process whose label has a resource closure.
