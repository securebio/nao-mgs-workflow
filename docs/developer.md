...
- If a DOWNSTREAM group has no hits, validation never runs for it, so the `createEmptyGroupOutputs` module writes its header-only `validation_hits` from the schema. It only does so for groups `PARTITION_TSV` found empty: a group whose validation failed publishes no `validation_hits` at all.
