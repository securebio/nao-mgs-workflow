...
- If an input to DOWNSTREAM has no data, the `createEmptyGroupOutputs` module will generate header-only TSV outputs and minimal empty JSON outputs based on schemas where available. Output files with no corresponding schema will be empty.
