...
- Steps pass a header-only input through as header-only output, so a DOWNSTREAM group with no hits runs the same processes as any other and publishes header-only tables. `PARTITION_TSV` writes a header-only `partition_empty_*` file for such a group rather than nothing, so it isn't dropped before validation.
