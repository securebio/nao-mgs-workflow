...
- Process conventions (see `modules/local/vsearch/main.nf` for an example of a well-written process that follows these conventions):
    - All processes should have a label specifying needed resources (e.g. `label "small"`). Resources are then specified in `configs/resources.config`.
        ...
        - When a process's peak memory scales strongly with input size, the label's `memory` directive may be a closure over the process inputs — e.g. `memory = { ResourceTierUtils.pickMemoryTier(reads, ...) }`. Any helper classes such a closure depends on must be declared at the top of `configs/resources.config`, not in `lib/*.groovy`: Nextflow's `lib/` auto-loading adds classes to the script classpath but not the config classpath, so a config-scope closure cannot resolve `lib/` classes. See `bbmask_resources` and `ResourceTierUtils` in `configs/resources.config` for an example.
