...
- `prim_align_ref_start_unclipped`: Location of the first reference base the read covers, with clipped bases counted as if they had aligned. `prim_align_ref_start` minus the alignment's leading clip (for paired-end data, mate 1's). Unlike `prim_align_ref_start`, it does not move when the aligner clips a read end.
...
Together each mate's two bounds give its unclipped 5′ coordinate, which is what `samtools markdup` keys duplicates on: the start for a mate aligned to the forward strand, the end for one aligned to the reverse strand.
