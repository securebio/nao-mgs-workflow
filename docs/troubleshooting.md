...
- Some processes (currently `DOWNLOAD_VIRAL_GENOMES`) also use `/scratch` as a per-task working directory via `process { withName: ... { scratch = '/scratch' } }` in `configs/profiles.config`. With this directive, the per-task working dir lives on the instance EBS volume rather than on Fusion/S3 for the duration of the task. SBD's standard Batch launch templates are sized for this; if you run on a custom launch template with a small root volume and hit `/scratch` permission or space issues, remove the `process { withName: <process> { scratch = '/scratch' } }` line for the affected process (this trades the speedup for slower Fusion-backed I/O).