...
## Scratch directories
- Processes labeled [`use_scratch`](https://docs.seqera.io/nextflow/reference/process#scratch) automatically use a local scratch directory for heavy file operations. Each profile in `configs/profiles.config` has a `withLabel: 'use_scratch'` selector that sets the appropriate scratch mode: `/scratch` on Batch profiles (where `aws.batch.volumes` mounts it from the host EBS), the system temp dir on `ec2_s3` (Fusion-enabled but no volume mount), or disabled on `ec2_local` (work dir is already local). SecureBio's standard Batch launch templates are sized for this; if you run on a custom launch template with a small root volume and hit `/scratch` space issues, you can remove the `withLabel: 'use_scratch'` block from the relevant profile in `configs/profiles.config` (this trades the speedup for slower Fusion-backed I/O).
