...
**Alignment-based marking** identifies duplicate reads on the basis of their assigned genome ID and alignment coordinates, as determined by Bowtie2 in the `RUN` workflow. In order to be considered duplicates, two read pairs must be mapped to the same genome ID by Bowtie2, with fragment coordinates that are within a user-specified distance of each other (default 1 nt) at both ends. A fragment runs from the leftmost mate's start coordinate to that coordinate plus `prim_align_fragment_length`, which is the template length Bowtie2 reported for the pair. Where Bowtie2 aligned the mates independently rather than as a pair (`prim_align_pair_status` of `UP`) it reports no template length, and the two mate start coordinates are used instead. This fuzzy matching allows for the identification of duplicate reads in the presence of small read errors, alignment errors or overzealous adapter trimming.

A read pair with only one mate aligned, or neither, is never grouped with any other read: a single coordinate, or none, is not evidence that two reads came from the same molecule. Such reads remain available to similarity-based marking, which can identify duplication among them from their sequences.
