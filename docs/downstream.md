...
It outputs a TSV for each sample group (`<group>_clade_counts.tsv.gz`) with eight columns:
...
5. `reads_direct_exemplar_total`: as `reads_direct_total`, but counting each read under the taxid of the exemplar representing it rather than its own
6. `reads_clade_total`: the number of reads assigned to the clade descended from the taxid (including the directly assigned reads) without deduplication
7. `reads_clade_dedup`: the number of reads assigned to the clade with deduplication
8. `reads_clade_exemplar_total`: as `reads_clade_total`, but counting each read under the taxid of the exemplar representing it

The `exemplar_total` columns differ from the `total` columns only where a duplicate group's members were assigned different taxa. They exist so that `dedup / exemplar_total` is a per-taxid duplication rate over one coherent set of reads, which `dedup / total` is not.
