...
Groups are built from reads of the same kind. A pair whose mates aligned to one genome on opposite strands (FR or RF), a pair whose mates aligned on the same strand (FF or RR), a pair split across two genomes, and a read with one mate unaligned each group only among themselves, since their coordinates describe different things. A read with neither mate aligned has no coordinates and is never a duplicate of anything.

A read with one mate unaligned is then attached to a complete pair that shares its 5′ coordinate and strand, as `samtools markdup` does, joining that pair's duplicate group and never acting as its exemplar. The attachment is one-way: the read joins the nearest such pair and only that one, so a coordinate shared with several pairs cannot merge them into one group. Reads left behind when their neighbours attach keep the group they were built into. A pair split across two genomes is keyed under both of them at once, so it is never an attachment target; `samtools markdup`, which keys each read against its own reference, would attach there too. An attached read counts towards the group's size but is left out of its pairwise match fraction, carrying one coordinate where the rest of the group has two.
