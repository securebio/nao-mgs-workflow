...
**Alignment-based marking** identifies duplicate reads on the basis of their assigned genome ID and alignment coordinates, as determined by Bowtie2 in the `RUN` workflow. In order to be considered duplicates, two read pairs must be mapped to the same genome ID by Bowtie2, with fragment coordinates that are within a user-specified distance of each other (default 1 nt) at both ends. The fragment runs from the leftmost mate start to that coordinate plus `prim_align_fragment_length`. This fuzzy matching allows for the identification of duplicate reads in the presence of small read errors, alignment errors or overzealous adapter trimming.
...
Both sets of columns are carried through validation into `{group}_validation_hits.tsv.gz`. Clade counting deduplicates on `prim_align_dup_exemplar` only, so its `reads_*_dedup` figures are alignment-deduplicated.
