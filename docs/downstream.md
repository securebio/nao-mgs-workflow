...
5. A **config file** in a clean launch directory, pointing to:
    ...
    - Parameters for BLAST validation:
        - `params.blast_db_prefix`: BLAST database name within the reference directory. Constant `"blast-db"` — INDEX publishes the DB (whatever `blast_db_name` was downloaded) under `results/blast-db/` with a `blast-db` alias, so downstream need not know the underlying DB name.
