...
**Alignment-based marking** identifies duplicate reads on the basis of their assigned genome ID, the unclipped 5′ coordinate of each aligned mate, and the strand each mate aligned to, all as determined by Bowtie2 in the `RUN` workflow. A mate's 5′ coordinate is the reference position of the end of the fragment that entered the sequencer, counting any clipped bases. `samtools markdup` keys duplicates on the same quantity.
...
Reads are only compared against reads of the same kind. A pair whose mates aligned to one genome on opposite strands (FR or RF), a pair whose mates aligned on the same strand (FF or RR), a pair split across two genomes, and a read with one mate unaligned will never be marked as duplicates of each other. A read with neither mate aligned has no coordinates and is never a duplicate of anything, where previously such reads all matched each other. This differs from `samtools markdup`, where a read with one mate unaligned could be marked as a duplicate of a complete pair, and where a read with neither mate aligned is not considered for duplicate marking at all.

For a pair whose mates are on opposite strands the two coordinates are held in strand order rather than coordinate order. That distinguishes FR from RF without the key having to decide which mate is leftmost — a decision that compares coordinates exactly, and so would separate duplicates whose mates sit within the tolerance of each other.
