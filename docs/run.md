...
E --> F(Kraken output)
D --> G[DOMAIN SUMMARY]
G --> H[Process domain summary output]
H --> I(Domain abundance output)
...
E --> G(Combined domain abundance reports)
...
To do this, reads from SUBSET_CLEAN are separated into ribosomal and non-ribosomal read groups using BBDuk, by searching for ribosomal k-mers from the SILVA database generated by the index workflow. The ribosomal and non-ribosomal reads are then passed separately to [TAXONOMY workflow](#taxonomic-assignment-taxonomy), which returns the Kraken2 reports and domain abundance estimates. These are then annotated and merged across samples to produce single output files.
