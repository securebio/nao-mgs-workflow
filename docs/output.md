...
- `{sample}_fastp.json`: Per-sample FASTP diagnostic data in JSON format, including read counts, quality metrics (Q20/Q30 rates), adapter statistics, and filtering results. Only produced for short-read (non-ONT) runs. Note that fastp's `overrepresented_sequences` keys are not published: fastp only measures them when passed `-p`, which we do not, so they were always empty. See `{sample}_qc_overrepresented_{stage}.tsv.gz` for overrepresented sequences as measured by FASTQC.
