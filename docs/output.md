...
- `virus-genome-metadata-gid.tsv.gz`: Genome metadata file generated during download of vertebrate viral genomes[^vertebrate] from viral Genbank, annotated additionally with Genome IDs used by Bowtie2 (allowing mapping between genome ID and taxid). Carries exactly one row per sequence present in `virus-genomes-masked.fasta.gz`: rows describing absent sequences are dropped, and where several assemblies package the same sequence the row kept is the one naming the smallest `assembly_accession`, which is the copy deduplication retained.
