...
- `{sample}_qc_overrepresented_raw.tsv.gz` and `{sample}_qc_overrepresented_cleaned.tsv.gz`: Overrepresented sequences identified by FASTQC for subset sample before (`raw`) and after (`cleaned`) adapter trimming, given as the sequence (`sequence`), the number of reads matching it (`n_occurrences`), and that count as a percentage of the sample's reads at that stage (`pc_reads`). Read these with FASTQC's counting rules in mind:
    - FASTQC counts **whole reads**, truncated to their first 50 bases. Reads longer than 50bp are therefore grouped by their leading 50bp, so two reads that differ only after base 50 are counted as one sequence. For ONT data, where reads are far longer than 50bp, this makes the table a summary of read *starts* rather than of whole reads.
    - Only sequences making up more than 0.1% of the sample's reads at that stage are listed, and at most the 100 most frequent are published. A file with only a header means FASTQC reported no overrepresented sequences.
    - Percentages are relative to that sample and stage's own read count, counted in **mates, not pairs** (paired reads reach FASTQC as a single interleaved file). A sequence present in every R1 and no R2 therefore reads as ~50%. The `raw` and `cleaned` stages can have different denominators, since trimming removes reads.
    - To bound memory, FASTQC only tracks sequences seen among the first 100,000 unique sequences in the file, so sequences that first appear late in a large file can be missed.
    - FASTQC's own "Possible Source" adapter/primer guess is not carried through by MultiQC and so is not published.
...
- `{sample}_fastp.json`: Per-sample FASTP diagnostic data in JSON format, including read counts, quality metrics (Q20/Q30 rates), adapter statistics, and filtering results. Only produced for short-read (non-ONT) runs. Note that the `overrepresented_sequences` keys in this file are always empty: fastp only runs its overrepresentation analysis when given `-p`, which we don't pass, and that analysis additionally needs seekable file input rather than the pipe the FASTP module uses. Treat those keys as "not measured" rather than "none found", and use `{sample}_qc_overrepresented_{stage}.tsv.gz` instead.
