...
def exclude_infections(
    ...
    Note: mutates `statuses` in place and returns the same object.
...
def include_infections(statuses: pd.Series, include_taxids: list[str]) -> pd.Series:
    """Mark each listed taxid as MATCH in-place (descendants are picked up by
    the later mark_descendant_infections() step)."""
    statuses[statuses.index.isin(set(include_taxids))] = MATCH
    return statuses
...
def check_infection(
    ...
    for misannotated taxa. If a taxon is included in hard_include_taxids,
    only the listed taxon itself is marked MATCH (its descendants pick up
    MATCH via the normal mark_descendant_infections() step below, exactly
    as they would for a Virus-Host-DB direct match). Hard-include is
    applied *after* hard-exclude, so manual includes win on conflict at
    the override target.
    Args:
        virus_taxids (pd.Series): Series of virus taxids to check.
        host_taxids (set[str]): Series of host taxids to check against.
        ...
        hard_include_taxids (list[str]): list of virus taxids to hard-include
            for host annotation (i.e. to force to mark as infecting), loaded
            from the per-host override JSON.
    Returns:
    ...
    # If no direct infections and no manual includes, short-circuit to INCONSISTENT
    if statuses.isin([MATCH]).sum() == 0 and not hard_include_taxids:
    ...
    # Include hard-included taxa (applied after exclude, so includes win on conflict)
    logger.info("\tInclude hardcoded infecting taxids.")
    statuses = include_infections(statuses, hard_include_taxids)
    # Expand to ancestors
...
def annotate_virus_db_single(
    ...
    Args:
        ...
        hard_exclude_taxids (list[str]): list of virus taxids to hard-exclude
            from host annotation (i.e. to force to mark as non-infecting).
        hard_include_taxids (list[str]): list of virus taxids to hard-include
            for this host (i.e. to force to mark as infecting).
    Returns:
...
def annotate_virus_db(
    ...
    Args:
        ...
        hard_exclude_taxids (list[str]): list of virus taxids to hard-exclude
            from host annotation (i.e. to force to mark as non-infecting).
        hard_include_mapping (dict[str, list[str]]): Mapping of host names
            to lists of virus taxids to hard-include for that host (i.e. to
            force to mark as infecting). Hosts absent from this mapping are
            treated as having no manual includes.
    Returns:
...
def main() -> None:
    ...
    _ = parser.add_argument(
        "host_infection_overrides",
        type=Path,
        help="Path to JSON file of per-host taxid overrides forcing MATCH (see ref/host-infection-overrides.json).",
    )
    _ = parser.add_argument(
